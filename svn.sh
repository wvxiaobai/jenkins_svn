...
then
  ...
  echo $last_vsersion
  echo $SVN_REVISION
  if [ $SVN_REVISION -eq $last_vsersion ] 
  then
     #svn没有更新，执行上次构建
  	 if [ -f "svn.txt" ]; 
     then
  ...
  then 
    ...
    tmp_log=`svn log $svn_url_tmp --username wangw --password mnLE8h10 --no-auth-cache -r $SVN_REVISION`
    IM_PRODUCTION_SVN_LOG=${tmp_log//-/}
    IM_PRODUCTION_SVN_LOG=${IM_PRODUCTION_SVN_LOG// /}
    echo $IM_PRODUCTION_SVN_LOG
    tmp_log_array=(${IM_PRODUCTION_SVN_LOG//|/ })  
    for var_tmp_log in ${!tmp_log_array[@]}
    do
      if [ $var_tmp_log -eq 1 ] 
  	  then
        IM_PRODUCTION_SVN_LOG=${tmp_log_array[var_tmp_log]}更新乐萌:
      fi
      
      if [ $var_tmp_log -eq 4 ] 
  	  then
        IM_PRODUCTION_SVN_LOG=$IM_PRODUCTION_SVN_LOG${tmp_log_array[var_tmp_log]}
      fi
    done
    echo $IM_PRODUCTION_SVN_LOG
	echo 'IM_PRODUCTION_SVN_LOG='$IM_PRODUCTION_SVN_LOG >$WORKSPACE/svn_update_log.txt
